...
public:
	...
	void andaDireita(float qtd){
		if(position.x + qtd + 50 < SCREEN_WIDTH) {
			position.x += qtd;
		}
	}

	void andaEsquerda(float qtd){
		if(position.x - qtd - 50 > 0) {
			position.x += qtd;
		}
	}

	void andaCima(float qtd){
		// adicionar pulo, com efeito subindo e descendo
...
int main()
{
    ...
    sprites.back().SetPosition(400.0f, 300.0f);    // posicionado em (300,200)
    sprites.back().SetScale(800.0f, 600.0f);       // 200×100 px
    sprites.back().SetRotation(0.0f);             // rotacionado 45°
    ...
    while (!glfwWindowShouldClose(window)) {
		...
		// Chama função com base na tecla pressionada
		if( glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
			move(sprites[1], window, 0.1f); // andaDireita
		} else if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
			move(sprites[1], window, -0.1f); // andaEsquerda
		} else if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
			// faz pular
			move(sprites[1], window, 1.0f); // andaCima
		} else if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
			move(sprites[1], window, -1.0f); // andaBaixo
		}
